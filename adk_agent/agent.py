...
def _allowlist_instruction_text() -> str:
    ...
    for key, action in config.ALLOWLIST.items():
        ...
        lines.append(f"- {key} (Tier {int(action.tier)}): {action.description} | params: {params}")
    return "\n".join(lines)


INSTRUCTION = f"""You are a database SRE diagnosis assistant for an Oracle
database. You may ONLY act by calling one of your provided tools -- never
